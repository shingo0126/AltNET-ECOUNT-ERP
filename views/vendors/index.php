...
<div class="stat-grid" style="grid-template-columns:repeat(2,1fr);">
    ...
    <div class="stat-card" style="display:flex;align-items:center;justify-content:center;gap:12px;">
        <a href="?page=vendors&action=export" class="btn btn-success"><i class="fas fa-file-csv"></i> CSV 다운로드</a>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="data-table">
                <thead>
                    <tr><th>#</th><th>업체명</th><th>담당자</th><th>연락처</th><th>이메일</th><th>주소</th><th class="text-center">관리</th></tr>
                </thead>
                <tbody>
                    <?php if (empty($vendors)): ?>
                    <tr><td colspan="7" class="text-center" style="padding:40px;color:var(--text-muted);">등록된 매입 업체가 없습니다.</td></tr>
                    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td><?= e($v['phone']) ?></td>
                        <td><?= e($v['email']) ?></td>
                        <td><?= e($v['address']) ?> <?= e($v['address_detail']) ?></td>
                        <td class="text-center">
                            <?php if (Auth::hasRole(['admin','manager'])): ?>
                            <button class="btn btn-outline btn-sm" onclick="editVendor(<?= htmlspecialchars(json_encode($v), ENT_QUOTES) ?>)"><i class="fas fa-edit"></i></button>
                            <a href="?page=vendors&action=delete&id=<?= $v['id'] ?>&token=<?= e(CSRF::generate()) ?>" class="btn btn-danger btn-sm" onclick="return confirmDelete('이 매입 업체를 삭제하시겠습니까?')"><i class="fas fa-trash"></i></a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
        ...
        <div class="pagination">
            ...
            <a href="?page=vendors&search=<?= urlencode($search) ?>&p=<?= $pag['current']-1 ?>"><i class="fas fa-chevron-left"></i></a>
            <?php endif; ?>
            <?php for ($i = max(1, $pag['current']-3); $i <= min($pag['total_pages'], $pag['current']+3); $i++): ?>
            <a href="?page=vendors&search=<?= urlencode($search) ?>&p=<?= $i ?>" class="<?= $i == $pag['current'] ? 'active' : '' ?>"><?= $i ?></a>
            <?php endfor; ?>
            <?php if ($pag['current'] < $pag['total_pages']): ?>
            <a href="?page=vendors&search=<?= urlencode($search) ?>&p=<?= $pag['current']+1 ?>"><i class="fas fa-chevron-right"></i></a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    </div>
...
<div class="modal-overlay" id="vendorModal">
    <div class="modal-box" style="max-width:580px;">
        <div class="modal-header">
            <h4 id="modal-title">매입 업체 등록</h4>
            <button class="modal-close" onclick="closeModal()">&times;</button>
        </div>
        <form method="POST" action="?page=vendors&action=save">
            <?= CSRF::field() ?>
...
<?php $pageScript = <<<'JS'
function openModal() {
    document.getElementById('modal-title').textContent = '매입 업체 등록';
...
function editVendor(v) {
    document.getElementById('modal-title').textContent = '매입 업체 수정';
    ...
    document.getElementById('f_phone').value = v.phone || '010-';
    document.getElementById('f_email').value = v.email || '';
    document.getElementById('f_zipcode').value = v.zipcode || '';
    document.getElementById('f_address').value = v.address || '';
    document.getElementById('f_address_detail').value = v.address_detail || '';
    document.getElementById('vendorModal').classList.add('active');
}
function closeModal() { document.getElementById('vendorModal').classList.remove('active'); }
initPhoneInputs();
JS;
?>
